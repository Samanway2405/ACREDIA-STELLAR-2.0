...
import { PublicCredentialCard, PublicCredentialCardData } from '@/components/student/PublicCredentialCard';
    ...
    useEffect(() => {
        ...
        async function fetchPublicCredential() {
                ...
                if (found) {
                    if (mounted) {
                        setCredential({
                            ...
                            credentialType: found.metadata?.credentialData?.credentialType || 'diploma',
            ...
            try {
                ...
                if (mounted) {
                    setError(err instanceof Error ? err.message : 'Failed to load public credential.');
                }
            } finally {
            ...
            <main className="container mx-auto px-4 py-12">
                <div className="max-w-4xl mx-auto space-y-8">
                    <div className="text-center space-y-2">
                        <h1 className="text-3xl font-bold text-foreground">Verified Academic Credential</h1>
                        <p className="text-muted-foreground">Public credential showcase backed by Stellar blockchain verification</p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <Card className="p-8 text-center space-y-4">
                            ...
                            <h2 className="text-2xl font-bold text-foreground">Credential Unavailable</h2>
                            <p className="text-muted-foreground">{error || 'This credential token could not be found.'}</p>
                            <Link href="/verify" className="inline-block mt-4 text-sm font-semibold text-primary underline">
